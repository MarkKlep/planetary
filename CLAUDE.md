...
- `rover.ts` — the LRV, built from primitives like `iss.ts` builds the station, merged to one buffer per material. Its **high-gain dish tracks Earth**, which the crews did by hand at every stop; it is also what keeps a metre of parabola out of the driver's face.
