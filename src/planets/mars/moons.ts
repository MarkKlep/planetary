...
// --- deterministic noise ---------------------------------------------------
// Seeded so the surfaces are identical on every load. A moon that re-rolled its
// craters on refresh would be a strange thing to look at twice.
...
function fbm(direction: Vector3, seed: number, octaves: number): number {
    let sum = 0;
    let amplitude = 1;
    let weight = 0;
    let frequency = 2.4;
